...
<head>
  ...
  <title>Review Reply Administration</title>
  <!-- base:css -->
        ...
        <div class="content-wrapper">
          <div class="row">
            <div class="col-lg-12 grid-margin stretch-card">
              <div class="card">
                <div class="card-body">
                  <h4 class="card-title">리뷰 댓글 관리</h4>
                      	...
                      	<c:if test="${reviewReplyCount > 0}">
                        	<c:forEach var="reviewReply" items="${reviewReplyList}">
                        		<tr>
                          			...
                          			<td>${reviewReply.nick}</td>
                          			<td>${reviewReply.content}</td>
                          			<td><fmt:formatDate value="${reviewReply.reg_date}" pattern="yyyy-MM-dd" /></td>
                         			<td>
				...
				<c:if test="${reviewReplyCount > 0}">
					<fmt:parseNumber var="pageCount" 
									 value="${reviewReplyCount / pageSize + (reviewReplyCount %  pageSize == 0 ? 0 : 1)}"
									 integerOnly="true" />
					...
					<nav class="blog-pagination justify-content-center d-flex">
                		<ul class="pagination">
                    		<c:if test="${startPage > 5}">
                        		<li class="page-item">
                        			<c:if test="${searchCount != 1}">
                            			<a href="/kanbu/admin/board/reviewReply.com?pageNum=${startPage-5}" 
                        	...
                        	<c:forEach var="i" begin="${startPage}" end="${endPage}">
                        		<li class="page-item">
                        			<c:if test="${searchCount != 1}">
                            			<a href="/kanbu/admin/board/reviewReply.com?pageNum=${i}" class="page-link">${i}</a>
                            		</c:if>
                            		<c:if test="${searchCount == 1}">
                        	...
                        	<c:if test="${endPage < pageCount}">
                        		<li class="page-item">
                        			<c:if test="${searchCount != 1}">
                            			<a href="/kanbu/admin/board/reviewReply.com?pageNum=${startPage+5}" class="page-link" aria-label="Next">
